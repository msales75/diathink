...
function validate() {
    ...
    var deadviews:{[i:string]:DeadView} = DeadView.viewList;
    ...
    var o:string, m:string, v:string, k:string, cp:string, cm:string;

    assert(View.currentPage instanceof PageView,
        "Page " + View.currentPage.id + " is not a PageView");
    assert(View.currentPage === views[View.currentPage.id],
        "Page " + View.currentPage.id + " is not in views list");
    for (o in outlines) {
        assert(outlines[o] instanceof OutlineRootView,
            "Outline ID " + o + " is not an OutlineRootView");
        assert(outlines[o] === views[o],
            "Outline ID " + o + " is not in the views list");
    }
    for (o in panels) {
    ...
    assert(OutlineNodeModel.root instanceof OutlineNodeModel,
        "Root model is not of correct type");
    assert(models[OutlineNodeModel.root.cid] === OutlineNodeModel.root,
        "Root is not in model list");
    for (m in models) {
    ...
    assert(actions.length === _.size(actions),
        "Actions.length is not right");
    if (actions.length > 0) {
        ...
        for (var i = 0; i < actions.length; ++i) {
            assert(i === (<Action>actions.at(i)).historyRank, "Action rank does not match historyRank id for i="+i);
            (<Action>actions.at(i)).validate();
        }
    } else {
        assert(lastaction === null,
